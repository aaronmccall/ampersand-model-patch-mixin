                ...
                if (collection) {
                    ...
                    if (collection) {
                        if (model.isNew() && ['add', 'replace'].indexOf(op.op) !== -1) {
                            ...
                            opArgs = [op.op, utils.makePath(propertyName, index === -1 ? collection.indexOf(model) : index, op.path), op.value];
                        }
                    }
                }
                if (child) {
                    if (model.isNew() && ['add', 'replace'].indexOf(op.op) !== -1) {
                        ...
                        opArgs = [op.op, utils.makePath(propertyName, op.path), op.value]
                    }
                }
            }
